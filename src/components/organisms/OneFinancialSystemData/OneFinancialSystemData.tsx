...
export const OneFinancialSystemData: React.FunctionComponent<Props> = ({
  main,
  systemNumber,
}): JSX.Element | null => {
  const { allBankRecords, forceUpdate, changeMainFsBetweenRecords } =
  ...
  const TitleBar: React.FunctionComponent = (): JSX.Element => (
    <div className="componentTitleBar">
      ...
      <div className="buttonsBar">
        <Button onClick={handleReset} title="Reset current financial system">
          Reset
        </Button>
        {bankRecord.documentType !== AcceptedDocuments.Document && (
          <Button
        ...
        {!main && (
          <Button className="hideButton" onClick={() => setIsVisible(false)}>
            X
          </Button>
        )}
      </div>
    ...
    <>
      {Object.entries(bankRecord)
        ...
        .map(([bankDetailsElementName, bankDetailsElementValue]) => {
          if (
            bankRecord.documentType === AcceptedDocuments.Document &&
            bankDetailsElementName === BankRecordElements.bankKey
          ) {
            return null;
          }
          if (
          ...
          return (
            <BankDetailsItem
              ...
              handleClick={(e: React.MouseEvent<HTMLElement>) =>
                pasteOneString(e, bankRecord, forceUpdate)
              }
            />
          );
        })}
